...
async def test_smtp_diagnostics_log_success_without_password(monkeypatch, caplog):
    ...
    assert any('stage=configuration result=success' in message and 'tls_mode=implicit_ssl' in message for message in messages)
    assert any('stage=connection result=success' in message for message in messages)
    assert any('stage=authentication result=success' in message for message in messages)
    assert any('stage=sendmail result=success' in message for message in messages)
    assert any('stage=delivery result=success' in message for message in messages)
